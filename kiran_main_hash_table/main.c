...
static int file_open(const char* path, struct fuse_file_info* fi)
{
    ...
    // Locate the file
    fh->node = resolve_path(path,0); // 0 because we dont want to create a we want to use the file.
    if(fh->node == NULL)
    {
        printf("Resolve_Path_ERROR\n\n");
        return -1;
    }
    printf("FILE OPENED\n");
    fi->fh = fh;    
...
int file_read(const char* path, char *buf, size_t size, off_t offset, struct fuse_file_info* fi)
{
